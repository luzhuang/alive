...
// @author mrdoob / http://mrdoob.com/
  ...
  Vector2D.prototype = {
    ...
    getGradient: function () {
      ...
      } else if (this.y > 0) { return Math.PI / 2 } else if (this.y < 0) {
        return -Math.PI / 2
      }
    },
  ...
  Span.prototype = {
    getValue: function (INT) {
      if (this.isArray) {
        ...
        if (!this.center) { return Proton.MathUtils.randomAToB(this.a, this.b, INT) } else {
  ...
  function FollowEmitter (mouseTarget, ease, pObj) {
    ...
    this.ease = Proton.Util.initValue(ease, 0.7)
...
// the own renderer
  ...
  CanvasRender.prototype.onParticleCreated = function (particle) {
    ...
    } else { particle.color = particle.color ? particle.color : '#ff0000' }
  }
  ...
  CanvasRender.prototype.onParticleUpdate = function (particle) {
    if (particle.target) {
      ...
      } else { this.context.fillStyle = particle.color }
  ...
  RectZone.prototype.crossing = function (particle) {
    if (this.crossType == 'dead') {
      ...
      if (particle.p.y + particle.radius < this.y) { particle.dead = true } else if (particle.p.y - particle.radius > this.y + this.height) {
        particle.dead = true
      }
    } else if (this.crossType == 'bound') {
      if (particle.p.x - particle.radius < this.x) {
